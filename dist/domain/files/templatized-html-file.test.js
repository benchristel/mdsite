...
import { TemplatizedMarkdownFile, replaceMarkdownHrefs, } from "./markdown-file.js";
...
test("TemplatizedHtmlFile", {
    "replaces absolute hrefs with relative ones"() {
        ...
        const file = new TemplatizedHtmlFile("/foo/bar.html", `<a href="/baz/kludge.html"></a>`);
        const [_, rendered] = file.render(project);
        expect(String(rendered), is, `<a href="../baz/kludge.html"></a>`);
    },
    "relativizes multiple hrefs"() {
        const project = new Project({}, "{{content}}");
        const file = new TemplatizedHtmlFile("/foo/bar.html", `<a href="/a/b.html"></a><a href="/foo/d.html"></a>`);
        const [_, rendered] = file.render(project);
    ...
    "leaves links in code tags alone"() {
        const project = new Project({}, "{{content}}");
        const file = new TemplatizedMarkdownFile("/foo/bar.md", '`<a href="/baz/kludge.html"></a>`');
        const [_, rendered] = file.render(project);
...
test("TemplatizedHtmlFile.title", {
    ...
    "extracts the title from an h1 tag with an id"() {
        expect(new TemplatizedHtmlFile("", `<h1 id="the-id">foo</h1>`).title, is, "foo");
    },
    "extracts the title from an h1 tag with child tags"() {
        expect(new TemplatizedHtmlFile("", `<h1><code>foo</code></h1>`).title, is, "foo");
    },
    "uses the title from the first h1 tag if there are several"() {
        expect(new TemplatizedHtmlFile("", `<h1>the title</h1><h1>not this</h1>`).title, is, "the title");
    },
    "defaults to the filename if there is no h1"() {
        expect(new TemplatizedHtmlFile("dir/file.html", `<p>whoa</p>`).title, is, "file.html");
    },
    "defaults to the filename if the h1 is empty"() {
        expect(new TemplatizedHtmlFile("file.html", `<h1></h1>`).title, is, "file.html");
    },
